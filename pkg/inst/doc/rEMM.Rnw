...
%% \VignetteIndexEntry{rEMM: Extensible Markov Model for Data Stream Clustering in R}
...
%\proglang{R}~extension package~\pkg{rEMM} which implements EMM on top of a 
%simple clustering algorithm for data streams and discuss some 
...
Such streams of continuously arriving
...
study the $k$-median problem. Their algorithm
...
$\{X_t\} = \langle X_1, X_2, \dots\rangle$ with $t$ being the time index. 
...
merged or deleted resulting in the evolving MC, the EMM.
...
on these structure which are necessary to add an EMM to an existing data stream 
...
Since the cluster represents a state in the EMM we need to
...
Conceptually, all transition counts in an EMM can be seen as a 
transition count matrix~$\mat{C}$ composed of all transition count vectors. 
It is easy to 
calculate the estimated transition probability matrix from the 
transition count matrix by
%\begin{equation}
$\mat{A} = \mat{C} / \vect{n},$
%\end{equation}
where $\vect{n} = (n_1, n_2,\dots,n_K)$ is the vector of row sums of $\mat{C}$.
Note that $n_i$ normally is the same as the 
...
the values of $\vect{n}$ calculated from $\mat{C}$ will diverge from the
...
helps to reduce space requirements significantly.
...
\item{\bf Adding a data point to an existing cluster.}
...
\item{\bf Creating a new cluster.}
This operation increases the number of clusters/state from $K$ to $K+1$ 
by adding a new \mbox{(micro-)cluster.}
To store the transition counts from an to this new cluster, 
...
\item Merge the outgoing edges for $s_m$  by $c_{mk} = c_{ik} +c_{jk},\;k=0,1,2,\ldots K$.
\item Merge the incoming edges for $s_m$ by $c_{km} = c_{ki} + c_{kj},\;k=0,1,2,\ldots K$.
...
\begin{align*}
c_{nk}& = n_n(c_{ik}/n_i),\;k=0,1,2,\ldots K\\
c_{kn}& = n_n(c_{ki}/n_i),\;k=0,1,2,\ldots K\\
c_{mk}& = n_m(c_{ik}/n_i),\;k=0,1,2,\ldots K\\
c_{km}& = n_m(c_{ki}/n_i),\;k=0,1,2,\ldots K\\ 
\end{align*}
After the split we delete $s_i$.

\item{\bf Fading the cluster structure.}
...
a learning rate $\lambda \ge 0$ to specify the weight over time. We
define the weight
...
However, finding canonical centroids 
...
{\em fixed pseudo medoids} and {\em moving pseudo centroids.}
...
As an alternative approach we define 
...
with single linkage make sense. Clustering micro-clusters results in
...
as a graph, and \pkg{Rgraphviz}~\citep{R:Rgraphviz:2009} for one of the
...
pertaining to the clustering and class~\class{EMMLayer} for the 
temporal aspects. Figure~\ref{fig:rEMM_classes} shows the UML class 
diagram~\citep{misc:Fowler:2004}.
\class{tNN} and \class{EMMLayer} are defined abstract since they are only
meant to be used together by \class{EMM}. However, the advantage of 
...
Class~\class{tNN} contains fields for all the clustering information
...
\item The state count vector $\vect{n}$ with the number of data points
currently assigned to each cluster.
\end{itemize}

Class~\class{EMMLayer} contains exclusively temporal information:
\begin{itemize}
...
initializes an empty clustering with an EMM layer. Several methods are defined
for the classes~\class{tNN} and \class{EMMLayer} and only methods which need
...
contains \class{tNN} and \class{EMMLayer}, all methods can directly be used
for \class{EMM} objects. The reason of separation is again flexibility for
future development.

The EMM layer information can be accessed using
\func{size} (number of states/clusters), 
\func{states} (name of states/clusters),
\func{current\_state} (get current state),
\func{transition} (access count or probability of a certain transition),
\func{transition\_matrix} (compute a transition count or probability
    matrix),
\func{initial\_transition} (get initial transition count vector).
To access information about the clustering,
we provide the functions 
\func{state\_counts} (number of observations assigned to each state/cluster) 
and \func{state\_centers} (centroids/medoids of clusters). 
...
internally the data points (rows) are processed sequentially. To speed up
processing, \func{build} directly manipulates the \class{graphNEL} 
data structure 
without using the functions provides in \pkg{graph}.
...
be updated. For convenience, a row of all \code{NA} in a sequence of
...
\func{build} has a learning rate parameter~\code{lambda}. 
...
of functions.  \func{remove\_states} and \func{remove\_transitions} remove
user specified states/clusters or transitions from the model.  To find rare
states or transitions  with a count below a specified threshold
\func{rare\_states} and \func{rare\_transitions} can be used.  \func{prune}
combines finding rare states or transitions and removing them into a
...
\func{merge\_states} which combines several clusters/states into a single
cluster/state.
...
can be supplied to \func{merge\_states} with \code{clustering=TRUE}
to create a reclustered EMM. Optionally new centers 
calculated by the clustering algorithm
can also be supplied to \func{merge\_states} as 
...
The helper function \func{find\_states} returns the
cluster/state sequence for given data points. The matching can be
nearest neighbor or exact. 
Nearest neighbor always returns a matching state, while exact will return no 
state (\code{NA}) if a data point does not fall within the threshold of any
cluster.
...
The number of data points represented by each state/cluster can be accessed
via \func{state\_counts}.
<<>>=
state_counts(emm)
@

State~2 has with a count of three the most assigned data points.
The state/cluster centers can be inspected using \func{state\_centers}. 
<<>>=
state_centers(emm)
@

\func{plot} for \class{EMM} objects provides several visualization methods.
For example as a graph.
...
more popular states and transitions are more prominently displayed.
...
To show the effect,
we train an EMM on the EMMTraffic data with a rather high learning
...
The states which were created earlier on (states with lower index number)
...
transitions can be pruned. For the example here, we prune all states 
...
The EMM visualized as a graph is shown in Figure~\ref{fig:sim_graph}(a).
The positions of the vertices of the graph (states) 
...
uses the relative position of the states
...
This results in the visualization in Figure~\ref{fig:sim_graph}(b) which shows
the same EMM graph but the relative position of the states is determined
...
directly used.
The size of the states and the width of
the arrows represent again state and transition counts.

We can also project the points in the data set 
into $2$-dimensional space and then add the centers of the states 
(see Figure~\ref{fig:sim_graph}(c)).
...
The simple graph representation in Figure~\ref{fig:sim_graph}(a) shows a rather
complicated graph for the EMM. However, 
Figure~\ref{fig:sim_graph}(b) with the vertices positioned
...
Figure~\ref{fig:sim_graph}(c) shows that the four groups represent the four
clusters in the data where the larger clusters are split into
more states (micro-clusters). We will introduce reclustering to simplify
...
length $l$ we have $l-1$ transitions. If we want to take the initial
transition probability also into account we extend the above equations
by the additional initial probability $a_{\epsilon,s(1)}$:
\begin{align}
...
also low. To analyze the problem we can look at the transition
...
sequence of states (counts and probabilities of zero above).  These missing
...
outside the threshold. The problem with missing transitions can be reduced by
...
From the summary we see that the average flows vary among catchments
...
Figure~\ref{fig:Derwent_state_counts} shows that the counts for the states have
a very skewed distribution with states~1 and 2 representing most observations
and states~5, 9, 10, 11, 12, 17, 19 and 20 being extremely rare.
...
Figure~\ref{fig:Derwent_EMM}(a) reveals that all states except state~11 and 12
are placed closely together.

Next we look at frequent states and transitions. 
We define rare here as all 
states/transitions that represent less than 0.5\% of the observations. 
On average this translates into less than two daily observation per year.
We calculate a count threshold,
use \func{prune} to remove rare states/transitions
and then we plot the pruned EMM.
...
states represent approximately 99.5\% of the river's behavior. 
All five states come from the lower half of the large cluster in 
Figure~\ref{fig:Derwent_EMM}(a).
States~1 and 2 are the most frequently used states and the wide bidirectional
arrow connecting them means that observing transitions between these 
two states are common.
%<<>>=
...
In Figure~\ref{fig:Derwent2}(a) we see that state~12 has a 
river flow in excess of
$100 m^3/s$ which only happened once in the observation period. State~11
...
flood occurs, the flow shoots up to $4 m^3/s$ which is caught by state~11.  The
...
Figure~\ref{fig:Mollicutes_graph}. Note that each state in the EMM corresponds
to one or more windows of the rRNA sequence (the size of the state indicates
the number of windows). The initial transition probabilities show that most
sequences start the first count window in state~1.  Several interesting
...
with EMM and to incorporate higher order models.


\section*{Acknowledgments}
The authors would like to thank the anonymous reviewers for their valuable
comments.
Part of the research presented in this paper was supported by
by NSF-IIS grant 0948893.


\bibliographystyle{abbrvnat}
\bibliography{emm,rna_sequences,stream_clust,misc}
\end{document}
